...
const EditJadwal = () => {
  ...
  // Ambil data jadwal saat komponen dimuat
  useEffect(() => {
    const fetchJadwalData = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:8000/api/web-dosen/jadwal/${id}/`, {
          headers: {
  ...
  const handleFormSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await axios.put(`http://localhost:8000/api/web-dosen/jadwal/${id}/`, {
        id: id, // Sertakan ID jika API memerlukannya
      ...
      console.error('Terjadi kesalahan saat mengedit jadwal:', error.response?.data || error.message);
      Swal.fire({
        icon: 'error',
        title: 'Kesalahan',
        text: 'Gagal mengedit jadwal!',
      });
    }
  };
...
export default withAuth(EditJadwal);
