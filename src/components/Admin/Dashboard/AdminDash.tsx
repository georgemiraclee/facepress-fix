...
import ChatCard from "../MatkulCard/index";
import TableAdmin from "../Tables/TableAdmin";
import CardDataStats from "../AdminCardDataStats";
...
const AdminDash: React.FC = () => {
  ...
  const [totalDosen, setTotalDosen] = useState<number | null>(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get("http://localhost:8000/api/web-admin/dashboard/", {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });

        const data = response.data.data; // Extract the nested data object

        setTotalMatkul(data.totalMataKuliah);
        setTotalMahasiswa(data.totalMahasiswa);
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2 md:gap-6 xl:grid-cols-3 2xl:gap-7.5">
        <CardDataStats title="Total Mata Kuliah" total={totalMatkul ? totalMatkul.toString() : "Loading..."} rate="">
          <svg
        ...
        <CardDataStats title="Total Mahasiswa" total={totalMahasiswa ? totalMahasiswa.toString() : "Loading..."} rate="">
          <svg
            className="fill-primary dark:fill-white"
            width="22"
            height="18"
        ...
        <CardDataStats title="Total Dosen" total={totalDosen ? totalDosen.toString() : "Loading..."}rate="">
        <svg
        className="fill-primary dark:fill-white"
        width="22"
        height="22"
