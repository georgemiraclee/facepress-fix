...
const DataPresensi = (): JSX.Element => {
  ...
  const fetchData = async () => {
    setIsFetching(true);
    try {
      const response = await axiosInstance.get(`http://localhost:8000/api/web-admin/presensi/${id}`);
      if (response.data) {
  ...
  const handleSetStatus = async (mahasiswaId: number, status: string) => {
    try {
      await axiosInstance.patch(`http://localhost:8000/api/web-admin/presensi/${mahasiswaId}/set-status/`, { status });
