...
const EditKelas = () => {
  ...
  useEffect(() => {
    ...
    const fetchKelasData = async () => {
      try {
        const response = await axiosInstance.get(`http://localhost:8000/api/mata-kuliah/kelas/${id}`);
  ...
  const handleFormSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await axiosInstance.put(`http://localhost:8000/api/mata-kuliah/kelas/${id}`, kelas);
